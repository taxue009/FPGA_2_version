...
static int fpga_probe(struct platform_device *dev)
{
	...
	/**
	void __iomem* aic_iccr;
	aic_iccr = (void __iomem*)(0xFEF78000) +0x87000 + 0x128;
	__raw_writel(0x00000004,aic_iccr);
	void __iomem* aic_iscr;
	aic_iscr = (void __iomem*)(0xFEF78000) +0x87000 + 0x12C;
	__raw_writel(0x00000004,aic_iscr);
	*/
	ret = request_irq(pin,fpga_interrupt,IRQ_TYPE_EDGE_FALLING | IRQ_NOAUTOEN ,"myfpga",dev);
	//ret = request_irq(pin,fpga_interrupt,IRQF_DISABLED | IRQF_TRIGGER_FALLING,"myfpga",dev);
	if(ret<0){
	  printk(KERN_ALERT "error\n");	
	}
	
	void __iomem* pio_ier;
	pio_ier = (void __iomem*)(0xFEF78000) +0x87400 + 0x40;
	__raw_writel(0x00100000,pio_ier);
